...
Last verified: 2026-05-07 with `git diff --check` and full VS Code test run (`117 passed`).
...
| US-001 Core Metric Calculation | Covered for current vertical slice | UnitTesting covers AC-001-1 through AC-001-6. SysTesting covers AC-001-1, AC-001-2, AC-001-3, AC-001-6, and AC-001-7 across Algorithm A/B/C paths, plus AC-001-8 for Algorithm B alive-subset patch replay. UserTesting covers Algorithm A maintainer workflow for the alive-at-endTime subset without public `--endRev`. | Broader non-happy-path algorithm workflows still belong to US-002 through US-010. |
...
| Algorithm A UserTesting | [tests/UserTesting/test_algorithm_a_user_workflow.py](tests/UserTesting/test_algorithm_a_user_workflow.py) | README_UserGuide Algorithm A local/remote Git maintainer workflows without public `--endRev`; US-001 / AC-001-8 alive-at-endTime subset; US-005 / AC-005-4 shallow Git history warning; US-009 Algorithm A live blame snapshot selection; Git remote auto-clone without `--repoPath`; CLI help contract after BASE `--endRev` removal |
...
3. Broader real-repository hardening: add provider-backed AlgA/AlgC fixtures, SVN merge-history blame fixtures, and reference-scale benchmarks beyond synthetic local repositories.
