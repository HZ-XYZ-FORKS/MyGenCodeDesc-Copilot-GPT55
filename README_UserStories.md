...
Last verified: 2026-05-07 with `git diff --check` and full VS Code test run (`115 passed`).
...
| US-005 Branch and History Conditions | Covered for current Algorithm B synthetic fixtures | Algorithm B UnitTesting covers outside-window origin exclusion, multi-merge uniqueness, long-lived branch origin filtering, shallow-history/replay-context limitation policy, and submodule gitlink exclusion policy. Root CLI SysTesting covers multi-merge uniqueness and submodule policy through `aggregateGenCodeDesc.py --algorithm B`. | Broader real-repository coverage for AlgA/AlgC, true shallow clone behavior, and VCS-provider-specific submodule/external workflows remain open. |
...
| Algorithm A UserTesting | [tests/UserTesting/test_algorithm_a_user_workflow.py](tests/UserTesting/test_algorithm_a_user_workflow.py) | README_UserGuide Algorithm A local Git maintainer workflow without public `--endRev`; US-001 / AC-001-8 alive-at-endTime subset; US-009 Algorithm A live blame snapshot selection; CLI help contract after BASE `--endRev` removal |
