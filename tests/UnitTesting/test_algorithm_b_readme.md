...
This test verifies the package-level Algorithm B patch replay path for US-001. It checks that v26.03 sparse DETAIL records join correctly with offline unified diffs, that omitted manual lines are treated as `genRatio=0`, that later patches replay over earlier snapshot state instead of being appended as independent additions, that rename metadata preserves attribution under final paths, that Git parent metadata and SVN numeric revisions control replay order, and that the ordered patch artifact is generated for audit output.
