...
This test verifies package-level Algorithm C validation for destructive and ordering-sensitive replay conditions. It focuses on rejecting a child revision whose `revisionTimestamp` is earlier than its parent revision, continuing with diagnostics when the ignore policy is selected, and rejecting parent chains that reference a missing genCodeDesc record.
...
- US-006 / AC-006-4: Algorithm C reports `clockSkewDetected` diagnostics and warning text when `onClockSkew=ignore` is selected.
