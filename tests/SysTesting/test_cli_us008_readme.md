...
This test verifies root `aggregateGenCodeDesc.py` behavior for the US-008 scale and performance slice. It focuses on empty-window zero metrics, documented scale policy in aggregate diagnostics, and safe abort behavior when a genCodeDesc file cannot be read mid-input with recoverable revision metadata.
...
- US-008 / AC-008-4 and US-010 / AC-010-4: genCodeDesc read failures abort clearly with file context, recover revision metadata from the filename stem, and write no partial output.
