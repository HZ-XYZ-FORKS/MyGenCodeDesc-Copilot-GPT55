...
This test verifies the `aggregateGenCodeDesc` CLI contract for US-001 using Algorithm A over v26.03 sparse metadata plus live Git blame, Algorithm B over v26.03 sparse metadata plus offline patch replay, and Algorithm C over v26.04 embedded-blame input. It checks that a real CLI invocation writes a v26.03-shaped aggregate JSON result with the expected metrics, writes the Algorithm B audit patch artifact, preserves Algorithm B rename attribution under final paths, and reports clear Algorithm B patch-directory errors.
...
- Algorithm B replay coverage: add-only patches, multi-patch delete/modify/add replay, multi-file/multi-hunk patch replay, pure rename, rename plus modify, chained rename, Git parent-before-child ordering, SVN numeric revision ordering, final snapshot denominator, ordered patch artifact output, and missing patch directory diagnostics.
