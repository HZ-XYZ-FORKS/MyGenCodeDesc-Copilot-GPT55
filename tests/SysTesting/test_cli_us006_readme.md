...
This test verifies the `aggregateGenCodeDesc` root CLI validation behavior for US-006 destructive and edge conditions, plus the first US-010 diagnostics/logging contract. It checks that missing input records, mismatched repository identity, duplicate revision IDs, invalid `genRatio` values, corrupted JSON, and AlgC parent/child clock skew fail explicitly before any aggregate output is written. It also covers Algorithm B's manual-attribution policy for patch revisions that lack genCodeDesc.
...
- US-006 / AC-006-1: empty `genCodeDescDir` is reported as missing genCodeDesc input for the current CLI boundary, and Algorithm B patch revisions without genCodeDesc are replayed as `Manual` / `genRatio 0` lines.
