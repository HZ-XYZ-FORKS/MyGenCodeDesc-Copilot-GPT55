...
This UserTesting file verifies the maintainer-facing Algorithm A workflow from `README_UserGuide.md`. It focuses on running the root CLI without the BASE-removed `--endRev` option, measuring the repository snapshot at `endTime`, auto-cloning a Git remote when `--repoPath` is omitted, and warning when shallow Git history can make blame partial.
