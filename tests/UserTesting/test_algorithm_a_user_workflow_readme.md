...
This UserTesting file verifies the maintainer-facing Algorithm A workflow from `README_UserGuide.md`. It focuses on running the root CLI without the BASE-removed `--endRev` option, measuring the repository snapshot at `endTime`, auto-cloning a Git remote when `--repoPath` is omitted, warning when shallow Git history can make blame partial, keeping SVN patch artifacts within `[startTime, endTime]`, and failing remote clone outages without partial outputs.
...
- US-009 / AC-009-3: remote Git clone failures for Algorithm A return runtime failure guidance and do not write aggregate or patch outputs.
...
5. Confirm the remote outage case exits with Algorithm A VCS guidance and leaves the output directory empty.